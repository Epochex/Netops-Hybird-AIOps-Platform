...
图：一次性 ablation 总结。Panel A 对比 invoke-all 与 topology-gated 的 LLM 请求量。Panel B 展示 efficiency-quality frontier：LCORE topology gate 从 invoke-all baseline 的 0% 调用减少，移动到 86.71% 调用减少，同时保留 100% high-value recall。Panel C 展示每条告警平均 selected 与 noise 节点数量，说明 noise 仍然保留为可审计证据，但不会进入核心 LLM evidence slice。
